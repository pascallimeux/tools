...
loginData = glob.glob("/tmp/test/%s/Library/Application Support/Google/Chrome/Profile*/Login Data" % os.path.expanduser("~"))
if len(loginData) == 0:
    loginData = glob.glob("/tmp/test/%s/Library/Application Support/Google/Chrome/Default/Login Data" % os.path.expanduser("~")) #attempt default profile
...
#print "safeStorageKey=%s" % (safeStorageKey)
safeStorageKey = "wTFBk3Cs+m2c7NkSI2w9ZA=="
