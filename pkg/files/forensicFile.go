...
var (
	massSign = []signFile{
		{`SQLite format 3`, `*.db`, `SQLite3 database`},
...
var SqliteSign = []signFile{
	{`53514C69746520666F726D6174203300`, `*.db`, `SQLite3 database`},
	{`SQLite format 3`, `*.db`, `SQLite3 database`},
}

//GetFileList get filename list of file in specifi format
func GetFileList(inputFL []string, signsFile []signFile) []string {

	fileList := []string{}
	for _, file := range inputFL {
...
//GetFileFormat get format of a file
func GetFileFormat(filepath string) (format string, err error) {
	...
	for _, val := range massSign {
		if strings.HasSuffix(filepath, val.SuffixFile) || bytes.Contains(f, []byte(val.Sign)) {
			format = val.FileFormat
			return
		}
	}
	return
